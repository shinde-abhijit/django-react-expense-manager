...
class Contact(models.Model):
    ...
    contact_image = models.ImageField(upload_to=rename_contact_image, blank=True, null=True)
    ...
    def clean(self):
        ...
        if self.first_name:
            if not re.fullmatch(r'[A-Za-z ]+', self.first_name):
                raise ValidationError({'first_name': "First name can only contain alphabets and spaces."})

        # last_name validation
        if self.last_name:
            if not re.fullmatch(r'[A-Za-z ]+', self.last_name):
                raise ValidationError({'last_name': "Last name can only contain alphabets and spaces."})

        # nickname validation
        if self.nickname:
            if not re.fullmatch(r'[A-Za-z ]+', self.nickname):
                raise ValidationError({'nickname': "Nickname can only contain alphabets and spaces."})

        # city validation
        if self.city:
            if not re.fullmatch(r'[A-Za-z ]+', self.city):
                raise ValidationError({'city': "City can only contain alphabets and spaces."})

        # state validation
        if self.state:
            if not re.fullmatch(r'[A-Za-z ]+', self.state):
                raise ValidationError({'state': "State can only contain alphabets and spaces."})

        # country validation
        if self.country:
            if not re.fullmatch(r'[A-Za-z ]+', self.country):
                raise ValidationError({'country': "Country can only contain alphabets and spaces."})
        ...
        if self.contact:
            if not re.fullmatch(r'\d{10,13}', self.contact):
                raise ValidationError({'contact': "Contact must be between 10 and 13 digits and contain only numbers."})
       ...
       # Ensure contact is unique for this user
        ...
        if self.email:
            ...
            if email_domain not in allowed_domains:
                raise ValidationError({'email': "Email must be from gmail.com, outlook.com, hotmail.com, or live.com."})
        ...
        if self.alternate_email:
            ...
            if alternate_email_domain not in allowed_domains:
                raise ValidationError({'alternate_email': "Alternate email must be from gmail.com, outlook.com, hotmail.com, or live.com."})

        # postal_code validation
        if self.postal_code:
            if not re.fullmatch(r'[A-Za-z0-9]{1,10}', self.postal_code):
                raise ValidationError({'postal_code': "Postal code must be up to 10 alphanumeric characters."})
